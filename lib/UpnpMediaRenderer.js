...
module.exports = class UpnpMediaRenderer extends EventEmitter {
	...
	_initDevice(deviceData) {
		...
		if (deviceInfo) {
			...
			device.client.on('status', (status) => {
				...
				this.realtime(device.deviceData, 'speaker_playing', device.state.TransportState === 'PLAYING');
				console.log('got status volume realtime', device.state.Volume);
		...
		this.mediaRenderer.once(`found:${deviceData.id}`, this._initDevice.bind(this, deviceData));
	}
	...
	setVolume(device, volume, callback) {
		device.client.setVolume(volume * 100, (err) => {
			console.log('setted volume', volume * 100, err);
			callback(err, volume);
		});
	}
